    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 mt-10 sm:mt-0">
      ...
      <div>
        {isLoaded ? (
          <>
            ...
            <p className="text-gray-500 mt-1 sm:mt-2 text-sm sm:text-base">{t.helperText}</p>
          </>
        ) : (
          <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6">
        {isLoaded ? (
          ...
          Array(4)
            .fill(0)
            .map((_, i) => <Skeleton key={i} width="w-full" height="h-20" className="rounded-2xl" />)
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {isLoaded ? (
          <>
            <ModuleCard icon={Bot} color="text-indigo-500" title={t.aiAssistant} description={t.aiAssistantDesc} />
            <ModuleCard icon={FileText} color="text-emerald-500" title={t.testGenerator} description={t.testGeneratorDesc} />
            <ModuleCard icon={CalendarDays} color="text-orange-500" title={t.studyPlanner} description={t.studyPlannerDesc} />
          </>
        ) : (
          Array(3)
            .fill(0)
            .map((_, i) => <Skeleton key={i} width="w-full" height="h-40" className="rounded-2xl" />)
        )}
      </div>
    </div>
  );
